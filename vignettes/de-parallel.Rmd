...
Now let's try it on the `DEanalysis` data set to see if there is
...
```{r, eval=FALSE}
res <- bench::mark(
  "sequential" = portfolio_compute_parallel(trx, mkt, plan = "sequential", method = "count"),
  "parallel" = portfolio_compute_parallel(trx, mkt, plan = "multisession", method = "count")
)
...
<img src="figures/de-parallel-bench1.PNG" align="center">
...
investors, when things get bigger it may be better to phisically
...
or in parallel by means of the 
...
<img src="figures/de-parallel-bench2.PNG" align="center">
...
optained by setting the `exact_market_prices` argument of 
